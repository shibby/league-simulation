...
class RandomizerUtil
{
     ...
     */
    public static function weightedRandom($values, $weights, $lookup = null, $total_weight = null)
    {
        if (null == $lookup) {
            list($lookup, $total_weight) = self::calcLookups($weights);
        }

        $r = mt_rand(0, $total_weight);

        return $values[self::binarySearch($r, $lookup)];
    }
     ...
     */
    public static function calcLookups($weights)
    {
        $lookup = array();
        $total_weight = 0;
     ...
     * @return int
     */
    public static function binarySearch($needle, $haystack)
    {
